...
const exports = {
  ...
  usedColNum: 11, // 新库是11列，比旧库少两列
  smallRowNum: 27,

  get colNum() {
    return this.usedColNum * this.divideCell; // 11 * 4 ,
  },
  get smallColNum() {
    return this.usedColNum * this.divideCell; // 小格子的总行列数
  },
  ...
  // 计算齿数
  toothMilliMeter: 15.708, // 一个齿的宽度，单位毫米
  safeGap: 20, // 顶部箱子和顶部横向运动小车的底部距离，单位毫米

  normalWidth: 9, //水平方向一格的宽度 36/4
  normalHeight: 6.75, // 一般货位高度是 27/4
  normalHeightBIG: 27, // 一般货位高度是 27
  topBoxNormalHeight: 26.073 / 4, // 最上面一行货位的高度是 26.073/4
  specialHeight: 18.927, // 底部特殊高度，18.927
  compensate: 19, // 方向改变的时候，齿数补偿，19 + 90度 同一个库里所有车同一个齿数

  specialBottomPart: 54 / 4, // 底部的特殊部分
  doubleBottomPart: 2 * 36 / 4, // 下降列底部的特殊部分是两个正常的宽度。
  specialTopPart: 18 / 4, // 顶部的特殊部分

  SUPPart: 27 / 4, // S形弯道上部分齿数
  SDownPart: 53 / 4, // S形弯道下部分齿数
  // 拣货台所占的大格子行数
  ...
  pickSitesPositionShift: {
    [PPconfig.pickSites.pickSiteA]: 0, // 第一个值关联pickSitesSmallRow的值
    [PPconfig.pickSites.pickSiteB]: - 0 - 53 / 4, //S形弯道下部分
  },
