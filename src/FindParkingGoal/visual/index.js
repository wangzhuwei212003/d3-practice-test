...
import {
  goalTable,
} from '../TestFunction/testConfig';

import {findParkingGoal} from '../TestFunction/findParkingGoal';
...
class Visual extends Component {
  ...
  testfindParkingGoal() {
    for (let i = 0; i < goalTable.length; i += 1) {
      console.log(`第${i}辆车的找到的空闲的目标位置：`, findParkingGoal(i));
    }
    // console.log(`第${1}辆车的找到的空闲的目标位置：`, findParkingGoal(1));
  }

  render() {
    return (
        <div ref={ele => this.grid = ele}>
          <p>货位算总齿数、actions，{BIGCELLTEXT.length}行{BIGCELLTEXT[0].length}列 <br/>
            给空闲的车找空闲的位置目标
          </p>
          <Button type="primary" onClick={() => this.testfindParkingGoal()}>testfindParkingGoal</Button>
          <br/>
