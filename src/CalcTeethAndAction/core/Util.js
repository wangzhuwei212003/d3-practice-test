...
export const sendVelocity = async function (path, shuttle, shift, endNodeArr, odom, teethAndAction, optIndex) {
  const LOGGER = shuttle.splitLogger;

  const SpecificActionsEnum = getEnums().SpecificActionsEnum;
  ...
  // 首先是要分别算出两个需要减速的距离
  let delayDist = delayGap * maxSpeed * timeGap / 1000; // 算上可能的延时（齿）
  if (
      haltHalfway &&
    ...
    delayDist += 4 * normalWidth;
  }

  let PSDist = PSBrakingDist(maxSpeed, PASS_PIN_Velocity, ACCELERATION) + delayDist; // 根据最大速度算出的 减速到过活门速度的刹车距离。
  let ToZeroDist = BrakingDist(maxSpeed, ACCELERATION) + delayDist; // 根据最大速度算出的 减速到 0 的刹车距离。
  // let PSDist = PSBrakeDis + delayDist;
  // let ToZeroDist = BrakeDis + delayDist;
  ...
  let distToStop_obj = calcTeeth(path.slice(0, ToZeroIndex), shift); // 改为0，因为 calcTeeth里已经去头了。shift标识当前位置的偏移。
  ...
  if (haltHalfway && distToStop <= ToZeroDist) {
    if (curSpeedRPM === 0) {
      ...
      let obUid = await findIdleUidOb(optIndex, LOGGER); // 当前小车、被挡住的小车的optIndex，
      ...
      if (obUid !== null) {
        ...
        await shuttles[obUid].cancelQueueChannel(); // 找到小车之后，给小车取消接收任务，直到小车到达目的地恢复任务。
    ...
    if (
        path[0][0] <= pickSitesSmallRow &&
        path[0][0] >= pickSitesSmallRow - pickSitesRowGap &&
        (path[0][1] === 0 || path[0][1] === colNum - 4)
    ) {
      // 如果不需要避障，在S形弯道的时候，减速是优先级最高
      ...
      if (distToGoal <= ToZeroDist) {
        ...
        shuttle.sendSpeedToShuttle(PASS_PIN_Velocity, 'SPEED_PRIORITY_AUTO_CONTROLLER');
        if (curSpeedRPM === 0 && distToGoal > 1) {
          // 如果速度是0，且没到终点，给一个活门速度
          if (!!showDispatchLog) LOGGER.warn('sendSpeedToShuttle,当前速度为0且没到终点，发送一个活门速度', PASS_PIN_Velocity);
          shuttle.sendSpeedToShuttle(PASS_PIN_Velocity, 'SPEED_PRIORITY_AUTO_CONTROLLER');
        }
      } else {
